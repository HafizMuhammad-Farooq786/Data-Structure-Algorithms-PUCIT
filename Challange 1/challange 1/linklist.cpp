...
public:
	...
	********************************************************************************************/
	...
	{
		return !first; 
	}//if there is no element first must be null and null means true
	...
	*/
	T deleteLast( ){
		...
		if ( first->link == null )
		{//there is only first element
			data = first->data;
			toDelete = first;
			first = null;
		}
		else
		{						//there exist 2 or more nodes in the list
	...
	*/
	bool deleteNode( T data )
	{
		...
		{//Minimum 2 Nodes exist because first->link is not null			
			...
			if ( temp->link )
			{				//means data found next to temp otherwise temp->link will be null
...
{
	srand( time( 0 ) );
	int i , choice , value,test[10];
	SLList<int> list;
	for ( i = 1; i <= 10; i++ )
	{
		choice = rand( ) % 2;
		test[i]=value = rand( ) % 90 + 10;
		if ( choice == 1 )
		{
			cout << "Adding " << value << " at Start\n";  list.addAtStart( value );
		}
		else			
		{
			cout << "Adding " << value << " at End\n";  list.addAtLast( value );
		}
	}
	list.showAll( );
	...
	for ( i = 1; i <= 8; i++ )
	{
		...
		if ( choice == 1 )
		{
			cout << "Deleting First Node having value:" << list.deleteFirst() << '\n';
		}
		else if ( choice == 2 )
		{
			cout << "Deleting Last Node having value:" << list.deleteLast() << '\n';
		}
		else if ( choice == 3 )
		{
			value = rand() % 90 + 10;
